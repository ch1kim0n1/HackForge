...
`;
  ...
  async installWebDependencies() {
    ...
    const manualSteps = [];
    if (fs.existsSync(path.join(backendPath, 'requirements.txt'))) {
      manualSteps.push('python3 -m venv venv && source venv/bin/activate && pip install -r requirements.txt');
    }
    if (fs.existsSync(path.join(backendPath, 'go.mod'))) {
      manualSteps.push('go mod download');
    }

    this.printManualInstallGuidance(backendPath, manualSteps);
  }
  ...
  async installSingleProjectDependencies(projectPath) {
    ...
    if (fs.existsSync(cargoPath)) {
      manualSteps.push('cargo build');
    }

    if (fs.existsSync(pubspecPath)) {
      manualSteps.push('flutter pub get');
    }

    this.printManualInstallGuidance(projectPath, manualSteps);
  }

  printManualInstallGuidance(projectPath, commands = []) {
    if (!Array.isArray(commands) || commands.length === 0) {
      return;
    }

    console.log(chalk.yellow('\n⚠ Non-JS dependency bootstrap was skipped.'));
    console.log(chalk.gray(`  Run manually in ${projectPath}:`));
